...
def _friendly_vocab_ai_error(exc: Exception) -> str:
    ...
    if "529" in text or "overloaded" in text:
        return (
            "The AI service is busy right now. Wait a few seconds, then tap "
            "Evaluate with AI again — your sentence is already saved."
        )
    if "429" in text or "rate" in text:
        return "Too many AI requests. Please try again in a minute."
    if "timeout" in text or "timed out" in text:
        return "The AI request timed out. Please try Evaluate with AI again."
...
def _looks_like_vietnamese(text: str) -> bool:
    """Heuristic: Step 3 answers must be English, not Vietnamese."""
...
def _vocab_english_required_feedback(
    *,
    translate_sentence: str,
    topic_sentence: str,
) -> Optional[Dict[str, Any]]:
    """Return structured feedback when the student typed Vietnamese in an English-only field."""

    bad_translate = bool(translate_sentence.strip()) and _looks_like_vietnamese(
        translate_sentence
    )
    bad_topic = bool(topic_sentence.strip()) and _looks_like_vietnamese(topic_sentence)
    if not bad_translate and not bad_topic:
        return None

    def _entry(sentence: str) -> Dict[str, Any]:
        return {
            "status": "fail",
            ...
            "recommendation": (
                "Viết câu bằng tiếng Anh và dùng từ mục tiêu. "
                "Ô này là bản dịch / câu tiếng Anh — không gõ tiếng Việt."
            ),
        }
    ...
    return {
        ...
        "unavailable_message": (
            f"Các ô {', '.join(parts)} cần câu tiếng Anh (English), có dùng từ vựng mục tiêu. "
            'Ví dụ Translate: dịch câu tiếng Việt ở đề sang English, không gõ "Tôi không biết".'
        ),
        "translate": _entry(translate_sentence),
        "topic": (
            _entry(topic_sentence)
            if topic_sentence.strip()
                ...
                "uses_target_word": False,
                "corrected_sentence": "",
                "recommendation": "Write an English sentence for the topic prompt.",
            }
        ),
        "band_style_tip": "",
        "step3_passed": False,
...
def _vocab_ai_unavailable_feedback(
    exc: Exception,
    *,
    translate_sentence: str,
    topic_sentence: str,
) -> Dict[str, Any]:
    message = _friendly_vocab_ai_error(exc)
    ...
    return {
        ...
        "translate": _entry(translate_sentence),
        "topic": _entry(topic_sentence) if topic_sentence.strip() else None,
        "band_style_tip": "",
    }
...
class LearningService:
    ...
    async def _check_vocab_ai_quota(
        ...
        return (
            False,
            "You've used all 10 free AI evaluations. Upgrade to a vocab plan for more.",
        )
    ...
    def _vocab_sentence_weakness_tags(
        self,
        *,
        invalid_language: bool,
        trans_score: Any,
        topic_score: Any,
        ...
        tags: List[str] = []
        if invalid_language:
            tags.append("invalid_language")
        if trans_score and not getattr(trans_score, "passed", False):
            tags.append("translation_failed")
        if topic_filled and topic_score and not getattr(topic_score, "passed", False):
            tags.append("topic_sentence_failed")
        if ai_feedback:
            translate = ai_feedback.get("translate") or {}
            topic = ai_feedback.get("topic") or {}
            if (
                translate.get("uses_target_word") is False
                or topic.get("uses_target_word") is False
            ):
                tags.append("missing_target_word")
            if (
                translate.get("status") == "needs_fix"
                or topic.get("status") == "needs_fix"
            ):
                tags.append("collocation_weak")
    ...
    async def submit_vocab_sentence(
        self,
        *,
        user_id: str,
        plan_code: str,
        ...
        word = await self._get_vocab_word(word_id)
        if not word:
            return {"word": None, "progress": None, "ai_feedback": None}

        stats = await self.stats.get_or_default(user_id)
        profile = stats.get("vocab_profile") or {}
        current_band = float(profile.get("current_band", 6.0))
        target_band = float(profile.get("target_band", 7.0))
        plan_code not in ("free", "guest")
        ai_requested = bool(use_ai)
        upgrade_hint: Optional[str] = None
        ai_quota_exceeded = False

        if ai_requested:
            allowed, hint = await self._check_vocab_ai_quota(user_id, plan_code)
            if not allowed:
                ai_requested = False
        ...
        translate_prompt = (
            word.get("vi_translate_prompt") or word.get("vi_prompt") or ""
        )
        topic_prompt = word.get("topic_prompt") or ""

        lang_feedback = _vocab_english_required_feedback(
            translate_sentence=translate_sentence,
            topic_sentence=topic_sentence,
        )
        invalid_language = lang_feedback is not None

        ai_feedback: Optional[Dict[str, Any]] = None
        ai_eval_failed = False
        if invalid_language:
            ai_feedback = lang_feedback
            ai_requested = False
        elif ai_requested:
            try:
                weakness_context: List[str] = []
                if self.personalization is not None:
                    from aiforen.domain.vocab_mission_priority import (
                        is_hygiene_weakness,
                    )

                    tops = await self.personalization.top_weaknesses(user_id, limit=3)
                    weakness_context = [
                        str(w.get("dimension") or "").strip()
                        for w in tops
                        if w.get("dimension") and not is_hygiene_weakness(w)
                    ][:2]
                ai_feedback = await get_llm_provider().evaluate_vocab_sentence(
                    word=word.get("word", ""),
                ...
                ai_eval_failed = True
                logger.error(
                    "vocab AI eval failed user={} word={} plan={} err_type={} err={}",
                    user_id,
                    word_id,
                    plan_code,
                    type(exc).__name__,
                ...
                ai_feedback = _vocab_ai_unavailable_feedback(
                    exc,
                    translate_sentence=translate_sentence,
                    topic_sentence=topic_sentence,
                )

        attempt = await self.vocab_attempts.insert(
            {
                "attempt_id": f"vatt_{secrets.token_urlsafe(10)}",
                "user_id": user_id,
                "word_id": word_id,
        ...
        pack_id = word.get("pack_id") or ""
        pack_total = await self._pack_total_words(pack_id) if pack_id else 12
        target = word.get("word") or ""
        translate_filled = bool(translate_sentence.strip())
        topic_filled = bool(topic_sentence.strip())

        trans_score = score_translate(
            sentence=translate_sentence,
            target_word=target,
            vi_prompt=translate_prompt,
            pack_total_words=pack_total,
            looks_like_vietnamese=_looks_like_vietnamese(translate_sentence),
        )
        topic_score = (
            score_topic(
                ...
                target_word=target,
                pack_total_words=pack_total,
                looks_like_vietnamese=_looks_like_vietnamese(topic_sentence),
            )
            if topic_filled
            else None
        )

        if ai_feedback and ai_requested and not ai_eval_failed:
            step3_passed = step3_passed_from_feedback(ai_feedback)
        else:
            step3_passed = trans_score.passed and (
                not topic_filled or (topic_score.passed if topic_score else False)
            )

        skip_mastery_bump = bool(
            invalid_language or (ai_retry and existing) or not trans_score.passed
        )
        mastery_delta = 0.0
        if skip_mastery_bump:
            state = (
                dict(existing)
                if existing
                else self._base_progress(
                    user_id=user_id, word_id=word_id, now=now, existing=existing
                )
            )
            self._stamp_vocab_progress(state, word)
            ...
            if trans_score.passed:
                prev_best = float(state.get("best_translate_pct") or 0)
                inc = max(0.0, trans_score.delta_pct - prev_best)
                if inc > 0:
                    state["best_translate_pct"] = trans_score.delta_pct
                    state["translate_passed"] = True
                    mastery_delta += await self._credit_pack_and_word(
                        user_id=user_id,
                        pack_id=pack_id,
                        state=state,
                        delta_pct=inc,
                ...
                next_step = self._bump_vocab_mastery(state)
            else:
                next_step = self._mastery_step_for(state)

            if topic_score and topic_score.passed:
                prev_topic = float(state.get("best_topic_pct") or 0)
                inc_topic = max(0.0, topic_score.delta_pct - prev_topic)
                if inc_topic > 0:
                    state["best_topic_pct"] = topic_score.delta_pct
                    state["topic_passed"] = True
                    mastery_delta += await self._credit_pack_and_word(
                        user_id=user_id,
                        pack_id=pack_id,
            ...
            intervals = {1: 2, 2: 4, 3: 7, 4: 21}
            interval = intervals.get(next_step, 0)
            state["last_sentence_id"] = attempt["attempt_id"]
            self._stamp_vocab_progress(state, word)
            state["last_studied"] = now
            state["updated_at"] = now
            state["spaced_repetition"] = {
                "ease_factor": 2.5,
                "interval": interval,
                "repetitions": next_step,
                "last_reviewed": now,
                "next_review": (
                    None if next_step >= 5 else now + timedelta(days=interval)
                ),
            }
            saved = await self.progress.upsert_vocab_state(
                user_id=user_id, word_id=word_id, update_doc=state
        ...
        weakness_tags = self._vocab_sentence_weakness_tags(
        ...
        await self._record_vocab_personalization(
            ...
            progress=saved,
            pack_mastery_pct=pack_pct,
            question_type=word.get("question_type"),
            step="sentence",
            is_correct=bool(step3_passed),
            score=float(trans_score.delta_pct or 0),
            time_taken=time_taken,
            answer_meta={
                "translate_filled": translate_filled,
                "topic_filled": topic_filled,
                "ai_requested": bool(use_ai),
                "ai_retry": bool(ai_retry),
            },
            ai_eval_meta={
                "ai_status": (ai_feedback or {}).get("ai_status"),
                "step3_passed": step3_passed,
                "translate_status": ((ai_feedback or {}).get("translate") or {}).get(
                    "status"
                ),
                "topic_status": ((ai_feedback or {}).get("topic") or {}).get("status"),
                "translate_quality": trans_score.quality,
                "topic_quality": topic_score.quality if topic_score else None,
            },
        ...
        return {
            ...
            "ai_locked": ai_quota_exceeded,
            "ai_quota_exceeded": ai_quota_exceeded,
            "upgrade_hint": upgrade_hint,
            "step3_passed": step3_passed,
            "mastery_delta_pct": round(mastery_delta, 2),
            "pack_mastery_pct": pack_pct,
            "translate_score": {
                "passed": trans_score.passed,
                "delta_pct": round(trans_score.delta_pct, 2),
                "quality": trans_score.quality,
                "reason": trans_score.reason,
            },
            "topic_score": (
                {
                    "passed": topic_score.passed,
                    "delta_pct": round(topic_score.delta_pct, 2),
                    "quality": topic_score.quality,
                    "reason": topic_score.reason,
                }
                if topic_score
                else None
            ),
